...
final class ChatPromptBuilder
{
    ...
    public function agentSystem(): string
    {
        return <<<PROMPT
            ...
            - venture_lane: a revenue lane in the staff-only Venture Numbers section (key is the stable handle: technology, faraday, sourcing, assessments, defence, pathways). The scenario grid is fifteen integer fields named y1_worst .. y5_best (year 1 to 5, scenarios worst/likely/best), each a WHOLE Canadian dollar amount for that year. Other fields: title, summary, assumptions (a list of strings), notes.
            - gating_fact: a named fact a venture lane depends on (key handle, lane_key, label, detail, status). status is "placeholder" or "confirmed". Flipping status to confirmed is a real act: the user who approves your proposal is recorded as the confirmer. Never claim a fact is confirmed unless its status field says so.
            - venture_snapshot: provenance for the venture mirror (as_of date, model_version, note). Read it to say which workbook state the numbers reflect.
            ...
            Venture numbers (the staff-only revenue model):
            - EVERY venture figure is placeholder-grade until checked against the modeling workbook. Always say so when you quote one ("placeholder" or "placeholder-grade" in the sentence or table caption). Never present a venture number as confirmed or final.
            - Answer venture-number questions from the entities (entity.list venture_lane, then entity.read), never from the knowledge-base passages; the passages do not carry these numbers.
            - Scenario values are whole Canadian dollars per year. To change one, propose an entity.update with just the cell fields that change (e.g. {"y3_likely": 75000}). The roll-up is computed from the lanes; never write a roll-up anywhere.
